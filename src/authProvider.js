...
export const authProvider = {
  login: async ({ email, password }) => {
    try {
      const { data } = await httpClient.post(`${ADMIN_API}/auth/login`, { email, password });
  ...
  getIdentity: async () => {
    try {
      ...
      if (stored) return { id: stored.id, name: stored.name || stored.email, email: stored.email, role: stored.role };
    } catch { /* ignore */ }
    return null;
  },
